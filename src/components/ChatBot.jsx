...
function ChatBot({ isOpen, onClose }) {
  ...
  const handleSendMessage = () => {
    ...
    if (
      inputMessage.toLowerCase().includes("book") ||
      inputMessage.toLowerCase().includes("appointment") ||
      inputMessage.toLowerCase().includes("reservation")
    ) {
      ...
      const matchedService =
        allServices.find((s) =>
          s.toLowerCase().includes(inputMessage.toLowerCase()),
        ) || inputMessage;
      ...
      botResponse.text =
        "I can help you with bookings, clinic hours, or information about our dental services. Just let me know what you're interested in!";
    }
        ...
        >
            ...
            <div className="flex items-center gap-3 relative z-10">
              ...
              <div>
                <h3 className="font-extrabold text-base leading-tight tracking-tight">
                  SMILE<span className="text-indigo-400">CARE</span> AI
                </h3>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50">
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`flex gap-2.5 max-w-[88%] ${msg.type === "bot" ? "flex-row" : "flex-row-reverse"}`}
                >
                  <div
                    className={`w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center ${msg.type === "bot" ? "bg-indigo-100 text-indigo-600 shadow-sm border border-indigo-100" : "bg-slate-950 text-white shadow-sm"}`}
                  >
                    {msg.type === "bot" ? (
                      <Sparkles size={14} />
                    ) : (
                  ...
                  <div
                    className={`p-3.5 rounded-2xl text-[13px] font-medium leading-relaxed shadow-sm ${
                      msg.type === "bot"
